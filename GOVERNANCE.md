...
## 8. PROJECT CONSTITUTION (A.G.E.N.T.S.)

These laws are absolute for the **A.G.E.N.T.S.** project and override general session preferences where they conflict.

| ID | Name | Severity | Description |
|----|------|----------|-------------|
| G1 | GATEKEEPER_SUPREMACY | ABSOLUTE | No action executes without Gatekeeper approval. |
| G2 | NO_LAYER_SKIPPING | CRITICAL | Every decision must pass through the defined agent chain (Aria -> wall-e). |
| G3 | ZERO_HALLUCINATION | CRITICAL | No fabrication of dates, costs, or reality checks. |
| G4 | AUDIT_FIRST | CRITICAL | Audit events must be recorded before state mutation. |
| G5 | FULL_TRACEABILITY | MANDATORY | Every action must carry a unique Trace-ID lineage. |
| G11| MOMENTUM_PROTECTION | MANDATORY | Decisions must account for project velocity and drag. |
