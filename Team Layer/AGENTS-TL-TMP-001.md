# Daily Huddle Minutes Template

| Field | Detail |
|---|---|
| **Document ID** | AGENTS-TL-TMP-001 |
| **Version** | 1.0.0 |
| **Status** | DRAFT |
| **Classification** | INTERNAL |
| **Owner** | Team Layer Lead |
| **Authority** | Chief Executive Officer and Gatekeeper |
| **Review Cycle** | Annual |

---

## 1. Purpose and Scope

This template provides the required structure for recording the minutes of the Daily Huddle (Toolbox Meeting) in accordance with the Daily Huddle SOP (AGENTS-TL-SOP-001).

---

## 2. Requirements or Procedures

*Copy the below template section precisely for each new daily huddle record.*

***

**Date:** YYYY-MM-DD  
**Chair:** [Lead Agent Name]  
**Participants:** [List participants]  

#### Completed Today
- [Agent] - [Completed item]

#### Outstanding Work
- [Agent] - [Item] - [Status]

#### Risks and Concerns
- [Risk] - [Impact] - [Owner]

#### Artifacts Produced
- [File path or artifact name]

#### Decisions and Approvals Needed
- [Decision] - [Approval required]

#### Next Day Priorities
- [Priority] - [Owner]

***

---

## 3. Records Management and Retention

Generated minutes must be saved to `Team Layer/huddles/YYYY-MM-DD.md`.

---

## 4. Approval and Authorisation

| Role | Name | Signature | Date |
|---|---|---|---|
| Team Layer Lead | | | |
| Chief Executive Officer | | | |
| Gatekeeper | | | |

---

### 6. Version Control

| Version | Date | Description of Changes | Author |
|---------|------------|-----------------------------------------|-------------------------|
| 1.0.0 | 11/04/2026 | Standardised template format applied. | Team Layer Lead |


